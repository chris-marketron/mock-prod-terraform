locals {
 ...
 bottlerocket_tags    = { "Name" = "${var.name}-node" }
...
data "aws_ssm_parameter" "bottlerocket_image_id" {
  name = "/aws/service/bottlerocket/aws-k8s-${var.nodegroup_version}/x86_64/latest/image_id"
}
...
resource "aws_launch_template" "bottlerocket_lt" {
  ...
  network_interfaces {
    associate_public_ip_address = true  # needs true because of the way the VPC was created...  :(
    delete_on_termination       = true
  }
